...
export default function Hero({ onSearchSubmit, currentCurrency }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 w-full max-w-5xl mx-auto px-4 sm:px-6 mt-8 sm:mt-11">
        <motion.div
          initial={{ opacity: 0, y: 25 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.85, delay: 0.45, ease: [0.16, 1, 0.3, 1] }}
          className="relative w-full bg-[#061210]/92 backdrop-blur-3xl border border-[#C6A868]/40 rounded-2xl sm:rounded-3xl px-4 sm:px-7 py-5 sm:py-6 shadow-[0_25px_70px_rgba(0,0,0,0.85),0_0_35px_rgba(198,168,104,0.12),inset_0_1px_2px_rgba(255,255,255,0.06)] transition-all duration-500 hover:border-[#C6A868]/60 before:absolute before:inset-x-12 before:top-0 before:h-[1.5px] before:bg-gradient-to-r before:from-transparent before:via-[#E2C98E]/80 before:to-transparent"
        >
          <div className="grid grid-cols-2 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] gap-4 md:gap-0 items-center">
            {metrics.map((metric, index) => {
              const Icon = metric.icon;
              return (
                <React.Fragment key={index}>
                  <div className="flex flex-col items-center justify-center text-center px-2 sm:px-4 py-1.5 group rounded-xl transition-all duration-300 hover:bg-[#0E201D]/40">
                    ...
                    <div className="relative mb-2.5">
                      ...
                      <div className="relative w-11 h-11 rounded-2xl bg-gradient-to-br from-[#152E28] to-[#0A1815] border border-[#C6A868]/40 flex items-center justify-center text-[#E0C58A] shadow-[0_4px_16px_rgba(0,0,0,0.6),inset_0_1px_1px_rgba(224,197,138,0.35)] group-hover:scale-110 group-hover:border-[#C6A868] transition-all duration-300">
                        <Icon className="w-5 h-5 stroke-[1.75]" />
                      </div>
                    </div>

                    {/* Animated Gold Numerals */}
                    ...
                    {/* Title Label */}
                    <div className="text-xs sm:text-[13px] font-sans font-semibold text-[#F5F1EA] tracking-wide mt-1 group-hover:text-[#D4BA7E] transition-colors leading-tight">
                      {metric.label}
                    </div>

                    {/* Institutional Micro-Narrative Subtitle */}
                    <div className="text-[10px] sm:text-[11px] font-sans text-[#8E9B97] font-light tracking-wide mt-0.5 leading-tight">
                      {metric.sublabel}
                    </div>
                  </div>

                  {/* Elegant Floating Gradient Divider on Desktop */}
                  {index < metrics.length - 1 && (
                    <div className="hidden md:block w-[1px] h-20 bg-gradient-to-b from-transparent via-[#C6A868]/30 to-transparent shrink-0 self-center" />
                  )}
                </React.Fragment>
              );
            })}
